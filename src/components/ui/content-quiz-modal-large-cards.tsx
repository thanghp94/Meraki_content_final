...
function MediaContent({ content, className = "", isCompact = false }: { content: QuestionContent; className?: string; isCompact?: boolean }) {
  switch (content.type) {
    case 'image':
      return (
        <img 
          ...
          style={{ 
            maxHeight: isCompact ? '100px' : '200px', 
            objectFit: 'cover',
    ...
    case 'video':
      return (
        <video 
          ...
          controls 
          className={`max-w-full h-auto rounded-lg ${className}`}
          style={{ maxHeight: isCompact ? '100px' : '200px' }}
        >
...
export default function ContentQuizModal({ 
  ...
  const hasImageAnswers = currentQuestion ? [1, 2, 3, 4].some(i => {
    const content = parseQuestionContent(getAnswerChoice(currentQuestion, i));
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-7xl w-full h-[90vh] p-0 bg-gray-50">
        <div className="flex flex-col h-full">
          ...
          <div className="flex flex-col bg-white border-b">
            ...
            {/* Compact Feedback Dialog - Below Progress Bar */}
            {showFeedback && (
              <div className={`mx-6 mb-4 p-3 rounded-lg border-2 ${
          ...
          {/* Content - Flex layout to push choices down */}
              ...
              <div className="w-full max-w-6xl mx-auto flex-1 flex flex-col">
                ...
                {questionContent.type === 'image' ? (
                  ...
                  <div className="mb-4 md:mb-8">
                    <div className="text-lg md:text-2xl font-bold text-blue-600 mb-4 md:mb-6">
                      <MediaContent content={questionContent} />
                    </div>
                  </div>
                )}

                {/* Spacer to push answer choices down */}
                <div className="flex-1"></div>

                {/* Answer Choices - Large Cards Layout */}
                <div className="w-full max-w-5xl mx-auto">
                  <div className={`grid gap-6 md:gap-8 ${
                    hasImageAnswers 
                      ? 'grid-cols-1 md:grid-cols-2' // Always 2 cols for images, large cards
                      : 'grid-cols-1 md:grid-cols-2' // 1 col on mobile, 2 on larger screens for text
                  }`}>
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={`p-6 md:p-8 rounded-lg border-2 text-left transition-all relative ${
                            hasImageAnswers ? 'min-h-[160px] md:min-h-[200px]' : 'min-h-[120px] md:min-h-[160px]'
                          } ${
                            showCorrect
                        ...
                        >
                          <div className="flex items-start gap-4 md:gap-6 h-full">
                            <div className={`flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full border-2 font-medium text-base md:text-lg flex-shrink-0 ${
                              showCorrect
                            ...
                            <div className="flex-1 flex items-center justify-center">
                              <MediaContent 
                                ...
                                isCompact={hasImageAnswers}
                              />
                            </div>
                            {showCorrect && (
                              <Check className="h-6 w-6 md:h-7 md:w-7 text-green-600 absolute top-4 md:top-6 right-4 md:right-6" />
                            )}
                            {showIncorrect && (
                              <X className="h-6 w-6 md:h-7 md:w-7 text-red-600 absolute top-4 md:top-6 right-4 md:right-6" />
                            )}
                          </div>
                        </button>
                      );
