    ...
    case 'image':
      return (
        <img 
          ...
          style={{ 
            maxHeight: isQuestionImage ? '100%' : (isCompact ? '60px' : '100px'),
            width: isQuestionImage ? '100%' : (isCompact ? 'auto' : '100%'),
...
export default function ContentQuizModal({ 
  ...
  // Check if current question has images in answers
  const hasImageAnswers = availableChoices.some(choice => {
    const content = parseQuestionContent(choice.content);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-7xl w-full h-[100vh] md:h-[90vh] p-0 bg-gray-50">
        <div className="flex flex-col h-full">
          {/* Header with Question Title, Navigation Dots, Action Buttons, Progress, and Close */}
          <div className="flex flex-col bg-white border-b flex-shrink-0">
            {/* Main Header Row */}
              ...
              <div className="flex items-center gap-4 md:gap-6">
                ...
                <div className="flex items-center gap-1 md:gap-2">
                  {questions.map((_, index) => (
                    <button
                      key={index}
                      className={`w-5 h-5 md:w-6 md:h-6 lg:w-8 lg:h-8 rounded-full flex items-center justify-center text-xs md:text-sm font-medium transition-all ${
              ...
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="w-8 md:w-12 lg:w-16 h-2 bg-gray-200 rounded-full">
                      <div 
            ...
            {/* Feedback Dialog - Below progress bar, not pushing content down */}
            {showFeedback && (
          ...
          <div className="flex-1 flex flex-col overflow-hidden min-h-0">
            {/* Question Content Area - Flexible height that responds to screen size */}
            <div className="flex-1 p-3 md:p-4 lg:p-6 overflow-hidden min-h-0">
              <div className="w-full h-full flex items-center justify-center">
                {questionContent.type === 'image' ? (
                  ...
                  <div className="text-base md:text-lg lg:text-2xl font-bold text-blue-600 text-center">
            ...
            {/* Answer Choices Area - Fixed height that adapts to screen size */}
            <div className="h-24 md:h-32 lg:h-40 p-3 md:p-4 lg:p-6 flex items-center justify-center flex-shrink-0">
              <div className="w-full max-w-6xl">
                {/* Always horizontal layout, but only show available choices */}
                <div className={`grid gap-2 md:gap-3 lg:gap-4 h-full ${
                  ...
                  {availableChoices.map((choice) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-2 md:p-3 lg:p-4 rounded-lg border-2 text-left transition-all relative h-full ${
                          showCorrect
                      ...
                      >
                        <div className="flex items-center gap-1 md:gap-2 lg:gap-3 h-full">
                          <div className={`flex items-center justify-center w-5 h-5 md:w-6 md:h-6 lg:w-8 lg:h-8 rounded-full border-2 font-medium text-xs md:text-sm flex-shrink-0 ${
                            showCorrect
                          ...
                          {showCorrect && (
                            <Check className="h-3 w-3 md:h-4 md:w-4 lg:h-5 lg:w-5 text-green-600 absolute top-1 right-1 md:top-2 md:right-2" />
                          )}
                          {showIncorrect && (
                            <X className="h-3 w-3 md:h-4 md:w-4 lg:h-5 lg:w-5 text-red-600 absolute top-1 right-1 md:top-2 md:right-2" />
                          )}
                        </div>
                      </button>
                    );
