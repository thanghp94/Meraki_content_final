    ...
    case 'image':
      return (
        <img 
          ...
          style={{ 
            maxHeight: isQuestionImage ? '100%' : (isCompact ? '80px' : '200px'),
    ...
    case 'video':
      return (
        <video 
          ...
          style={{ maxHeight: isQuestionImage ? '100%' : (isCompact ? '80px' : '200px') }}
        >
      ...
      <DialogContent className="max-w-7xl w-full h-[90vh] p-0 bg-gray-50">
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 flex flex-col overflow-hidden relative">
            {/* Feedback Overlay - Above content, not pushing it down */}
            {showFeedback && (
              <div className="absolute top-4 left-4 right-4 z-10">
                <div className={`p-3 rounded-lg border-2 shadow-lg ${
            ...
            <div className="flex-1 flex flex-col p-4 md:p-8 overflow-hidden">
              <div className="w-full max-w-6xl mx-auto flex-1 flex flex-col">
                {/* Question Content - Responsive container */}
                <div className="flex-1 flex items-center justify-center mb-6">
                  {questionContent.type === 'image' ? (
                    ...
                    <div className="text-lg md:text-2xl font-bold text-blue-600 text-center">
                      <MediaContent content={questionContent} />
                    </div>
                  )}
                </div>

                {/* Answer Choices - Fixed at bottom */}
                <div className="w-full">
                  <div className={`grid gap-4 md:gap-6 ${
                    hasImageAnswers 
                      ? 'grid-cols-4' // 4 cards in a row for images
                      : 'grid-cols-1 md:grid-cols-2' // 1 col on mobile, 2x2 on larger screens for text
                  }`}>
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={`p-4 md:p-6 rounded-lg border-2 text-left transition-all relative ${
                            hasImageAnswers ? 'min-h-[120px] md:min-h-[140px]' : 'min-h-[120px] md:min-h-[160px]'
                          } ${
                            showCorrect
                        ...
                        >
                          <div className="flex items-start gap-3 md:gap-4 h-full">
                            <div className={`flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-full border-2 font-medium text-sm md:text-base flex-shrink-0 ${
                              showCorrect
                            ...
                            <div className="flex-1 flex items-center justify-center">
                              <MediaContent 
                                ...
                                className={`font-medium ${hasImageAnswers ? 'text-sm md:text-base' : 'text-base md:text-lg'}`}
                                isCompact={hasImageAnswers}
                              />
                            </div>
                            {showCorrect && (
                              <Check className="h-5 w-5 md:h-6 md:w-6 text-green-600 absolute top-3 md:top-4 right-3 md:right-4" />
                            )}
                            {showIncorrect && (
                              <X className="h-5 w-5 md:h-6 md:w-6 text-red-600 absolute top-3 md:top-4 right-3 md:right-4" />
                            )}
                          </div>
                        </button>
                      );
