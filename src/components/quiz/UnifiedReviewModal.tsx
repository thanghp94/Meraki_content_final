...
export default function UnifiedReviewModal({ isOpen, onClose, content, contentItems }: UnifiedReviewModalProps) {
  ...
  const fetchReviewItems = async () => {
    try {
      ...
      // Determine which content items to process
      const itemsToProcess = contentItems && contentItems.length > 0 ? contentItems : [content];
      console.log('Processing', itemsToProcess.length, 'content items for review');

      // Fetch vocabulary if enabled
      if (settings.includeVocabulary) {
        console.log('=== VOCABULARY DEBUG START ===');
        
        // Collect all vocabulary words from all content items
        ...
        itemsToProcess.forEach((item, index) => {
          ...
          const vocabularyWords = extractVocabularyWords(item.infor1 || '');
          console.log(`Extracted vocabulary words from item ${index + 1}:`, vocabularyWords);
          
          allVocabularyWords.push(...vocabularyWords);
        });

        // Remove duplicates
        const uniqueVocabularyWords = [...new Set(allVocabularyWords)];
        console.log('All unique vocabulary words:', uniqueVocabularyWords);
        console.log('Total unique vocabulary words:', uniqueVocabularyWords.length);
        
        if (uniqueVocabularyWords.length > 0) {
          console.log('Making API call to /api/vocabulary/search with words:', uniqueVocabularyWords);
          ...
          try {
            const response = await fetch('/api/vocabulary/search', {
              ...
              body: JSON.stringify({ words: uniqueVocabularyWords }),
            });
          ...
          console.log('No vocabulary words extracted from any content items');
        }
      ...
      if (settings.includeQuestions) {
        console.log('=== QUESTIONS DEBUG START ===');
  ...
  const renderVocabularyItem = (vocab: VocabularyItem) => (
    <div className="flex-1 flex flex-col">
      {/* Flashcard - Dynamic layout based on showContent state */}
      <div className={`flex-1 transition-all duration-500 ${showContent ? 'flex' : 'flex flex-col items-center justify-center'}`}>
        {!showContent ? (
                ...
                {showAnswer && (
                  <div className="space-y-4 animate-in fade-in-50 slide-in-from-bottom-4 duration-300">
                    <div className="p-6 bg-blue-50 rounded-lg">
                      ...
                      <p className="text-gray-700 text-lg leading-relaxed">{vocab.definition}</p>
                    </div>
                    
                    {vocab.exampleSentence && (
                      <div className="p-6 bg-green-50 rounded-lg">
                        <h3 className="font-semibold text-green-800 mb-3 text-lg">Example</h3>
                        <p className="text-gray-700 italic text-lg leading-relaxed">"{vocab.exampleSentence}"</p>
                      </div>
                    )}
                ...
                <div className="mb-8">
                  <div className="text-center mb-6">
                    <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-6 border-l-4 border-blue-500">
                      <div className="text-2xl md:text-3xl font-bold text-gray-800 leading-relaxed">
                ...
                {showAnswer && (
                  <div className="mt-6 space-y-4 animate-in fade-in-50 slide-in-from-bottom-4 duration-300">
                    <div className="p-4 bg-green-50 rounded-lg border-l-4 border-green-500">
                      ...
                      {question.answer ? (
                        // Use the answer field if it exists
                        <p className="text-gray-700">{question.answer}</p>
                      ) : choices.length > 0 && question.correct_choice ? (
                        // Fall back to multiple choice logic
                        ...
                        <p className="text-gray-700">No answer available</p>
                      )}
                      
                      {selectedChoice && choices.length > 0 && question.correct_choice && (
